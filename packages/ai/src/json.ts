...
export function toCanonicalJsonValue(value: unknown): CanonicalJsonValue {
  ...
  if (isPlainObject(value)) {
    ...
    const canonicalEntries = sortedKeys.map((key) => [key, toCanonicalJsonValue(value[key])]);
...
function stableStringifyCanonicalValue(value: CanonicalJsonValue): string {
  ...
  if (Array.isArray(value)) {
    return `[${value.map((entry) => stableStringifyCanonicalValue(entry)).join(",")}]`;
  }
  ...
  const members = keys.map((key) => {
    ...
    if (entry === undefined) {
      throw new TypeError(`Canonical JSON object is missing key "${key}".`);
    }
    return `${JSON.stringify(key)}:${stableStringifyCanonicalValue(entry)}`;
  });
  return `{${members.join(",")}}`;
}
...
function sha256Bytes(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      messageSchedule[index] = view.getUint32(offset + index * 4, false);
    }

    for (let index = 16; index < 64; index += 1) {
      const value15 = messageSchedule[index - 15] ?? 0;
      const value2 = messageSchedule[index - 2] ?? 0;
      ...
      messageSchedule[index] =
        ((messageSchedule[index - 16] ?? 0) +
          sigma0 +
          (messageSchedule[index - 7] ?? 0) +
          sigma1) >>>
        0;
    }
...
export function stableStringifyJson(value: unknown): string {
  return stableStringifyCanonicalValue(toCanonicalJsonValue(value));
}
